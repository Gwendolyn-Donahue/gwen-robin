...
#'   savetofolder = "C:/Projects/Datasets/DO/Continuous/Greenwood/2019")
#' }
#' @export
  ...
  if (!is.null(savetofolder)){
    ...
    for (i in dofiles){
      ...
      doreformat=data.frame("Unix.Timestamp"=as.character(datetime),
                            ...
                            #"Central.Standard.Time"=as.character(lubridate::with_tz(datetimeutc,'US/Central')),
                            #"Pacific.Standard.Time"=as.character(lubridate::with_tz(datetimeutc,'US/Pacific')),
                            "Pacific.Standard.Time"=as.character(lubridate::with_tz(datetimeutc,"America/Los_Angeles")),
    ...
    dorowadd=data.frame("Unix.Timestamp"=as.character("(Second)"),
                        ...
                        "Pacific.Standard.Time"=as.character("(none)"),
    ...
    date_start=unlist(strsplit(folder,split="-"))[1]
    
    if (substr(savetofolder,(nchar(savetofolder)),nchar(savetofolder))!="/"){
      savetofolder=paste0(savetofolder,"/")
    }
    
    pathout=paste0(savetofolder, date_start, "-", serialnum,".csv")
