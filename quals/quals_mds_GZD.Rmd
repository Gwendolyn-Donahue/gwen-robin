...
chnms_coords <- read_excel("/Users/gwendolyndonahue/Documents/GitHub/gwen-robin/quals/CHNMS_Site_Notes.xlsx")
```
## phylum assigning
```{r}
...
```
# CHNMS 2024 report plots
## Simple map with coords
```{r}
library("sf")
...
chnms_coords_limited <- chnms_coords %>% 
  mutate("Site Type" = ifelse(Dive_site == TRUE, "Dive Site", "Non Dive Site")) %>%
  select(c(Site_name, "Site Type", Latitude_2024, Longitude_2024)) %>% 
  rename(Latitude=Latitude_2024, Longitude=Longitude_2024)

loadfonts(device = "mac")

muted_rainbow <- c( "#8B0000", "#1E90FF", "#006400", "#FF8C00", "#B8860B",
  "#8B4513", "#C71585", "#00008B", "#228B22", "#D2691E", 
  "#9ACD32", "#CD5C5C", "#A52A2A", "#FFD700", "#2F4F4F", 
  "#4B0082", "#8A2BE2", "#B22222", "#20B2AA")

ggplot(data = world) +
    geom_sf(fill = "blue") +  
    geom_sf(data = world, fill = "gray17") +  
    geom_point(data = chnms_coords_limited, 
               aes(x = Longitude, y = Latitude, 
                   color = Site_name, 
                   shape = `Site Type`),  
               size = 3) +  
    coord_sf(xlim = c(-121.5, -119.5), 
             ylim = c(34.3, 35.6),        
             expand = FALSE) +  
    scale_color_manual(values = muted_rainbow) +  
    scale_shape_manual(values = c("Dive Site" = 16, "Non Dive Site" = 15)) +  
    theme_minimal() + 
    theme(legend.title = element_blank(), 
          legend.position = "none", 
          legend.key = element_blank(),
          legend.box.background = element_blank(),
          text = element_text(family = "Times New Roman", size = 16, color = "#000000"))
ggsave("/Users/gwendolyndonahue/Desktop/chnms2024_coordmap.png", 
       width=12, height=7)
```
# All CHNMS plots
...
#go thru each region, find species accum, add that df to list
for (region in names(test_matrix_list)) {
  mat <- test_matrix_list[[region]] #current region's matrix
  accum <- specaccum(mat, method = "rarefaction") #calculate species accum
  # turn results into a df
  accum_df <- data.frame( 
    Site = accum$sites,       
    Species = accum$richness,  
    Region = region)
  # add to  list
